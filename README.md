...
### 模块一：文档库
本模块采用类Zotero的文献库管理设计，**上传论文pdf文档至系统，自动完成解析、清洗、分块和向量化存储。支持文件夹组织管理、文献条目的批量上传、拖拽分类与删除**。
...
### 模块三：知识问答
本模块负责提供类网页AI对话式的知识库问答功能，你可以询问任何与论文文献相关的问题。RAG技术保证了文献库的数量可以扩充得足够大，检索召回和Rerank环节能保证检索到最符合问题的片段，然后交给Deepseek深度思考，最终回答你的问题。

已要求Deepseek严格参考文献片段回答，最大程度降低幻觉。如果文献库中不包含相关信息，Deepseek会直接说明缺乏参考信息。
...
#### 3.配置环境变量
将 `config/example.env` 复制并重命名为 `config/.env`，然后填写对应的 API Key
```bash
1.DEEPSEEK_API_KEY=
...
| 路径 | 说明 |
|------|------|
| `data/papers/` | 原始 PDF 文件 | （论文的原始pdf与清洗后的chunks.json文件存储在此处，每个论文使用单独文件夹存放）
